...
$\hat \beta_1 = \frac{\sum (x_i - \bar x) (y_i - \bar y)}{\sum (x_i - \bar x)^2}$, where 
$\hat \beta_n$ represents an estimated value and $\bar x$ represents the mean of the set $x$
...
$\hat \beta_1 = \frac{1806757.14}{322280.86} = 5.60$ and $\hat \beta_0 = 2257.14 - 5.61 \cdot 512.86 = -618.01$

So the regression line can be expressed as $\hat y = 5.60 x -618.01$
...
linear regression as the right side is linear in the $\beta$ terms (if we were to let
